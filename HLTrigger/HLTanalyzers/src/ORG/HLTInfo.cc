...
void HLTInfo::analyze(const edm::Handle<edm::TriggerResults>                 & hltresults,
  ...
  if (hltresults.isValid()) {
    ...
    for (int itrig = 0; itrig != ntrigs; ++itrig){
      ...
      //MODIFIED 12-10-13 TO INCLUDE TRIGGER DECISION PT FOR JET TRIGS
      for(unsigned int ihlt=0; ihlt<hltConfig_.moduleLabels(trigName).size(); ihlt++){
          const unsigned int filterIndex(triggerEventHandle_->filterIndex(edm::InputTag(hltConfig_.moduleLabels(trigName).at(ihlt),"",processName_)));
          if (filterIndex<triggerEventHandle_->sizeFilters()) {
              ...
              for (trigger::size_type i=0; i!=n; ++i) {
                  ...
                  //TO.id > 0 gets lots of trigger primitives for more difficult triggers, but leads to problems with jet triggers as it gets uncalibrated trigger pt's
                  //VIDS>0 ensures that the object is really reconstructed but doesn't work for the photon_NoCaloID triggers, amongst others

                  if(VIDS[i]>0 || TO.id()>=0){ // Leading jet for decision of module: hltSingleJetXXRegionalNoJetID (amongst others)
                    trigObject[itrig].push_back(TO);
                    //if(trigPt[itrig]<TO.pt()) trigPt[itrig] = TO.pt();
                  
                    //cout << "Trigger name: "<< trigName << endl;
                    //cout << "module label: " << hltConfig_.moduleLabels(trigName).at(ihlt) << endl;
